...
export async function getWatchedVideos(token, userId) {
	...
	try {
		...
		console.error('getWatchedVideos GraphQL request error:', error);
		throw error;
	}
}

export async function getFavoritedVideos(token, userId) {
	const operationsDoc = `
	...
	try {
		...
		console.error('getFavoritedVideos GraphQL request error:', error);
		throw error;
	}
}
